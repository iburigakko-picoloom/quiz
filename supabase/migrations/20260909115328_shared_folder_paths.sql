-- Folder snapshots remain protected by the existing shared-set RLS policies.
alter table public.shared_problem_sets add column if not exists folder_path jsonb not null default '[]'::jsonb check (jsonb_typeof(folder_path) = 'array' and jsonb_array_length(folder_path) <= 2);
grant select (folder_path) on public.shared_problem_sets to anon, authenticated;
...
begin
  ...
  return jsonb_build_object(
    ...
    'folder_path', set_row.folder_path,
