...
export function StudyRecordScreen({ data, onBack, onStart }: { data: AppData; onBack: () => void; onStart: (session: QuizSession) => void }) {
  ...
  return <Layout><div className="study-record">
    ...
    <main className="study-record__body">
      <section className="study-record__card study-record__card--review" aria-labelledby="study-review-title">
        <h2 id="study-review-title">復習するべき問題</h2>
        {count ? <>
          <div className="study-record__recommendation">
            <p className="study-record__muted study-record__recommendation-label">今日のおすすめ</p>
            <p className="study-record__recommended-count"><strong>{count}</strong>問</p>
          </div>
          <p className="study-record__breakdown"><span className="study-record__breakdown-due">期限到来 {recommended.dueCount}問</span><span className="study-record__breakdown-check">要確認 {recommended.needsCheckCount}問</span></p>
