...
export function UsageGuideExample({ stage }: { stage: GuideExample }) {
  ...
  return <div className={`usage-guide__example${chat ? ' usage-guide__example--chat' : ''}`} inert>
    ...
    {stage === 'overview' && <><h3>詳細解説一覧</h3><p>月の満ち欠け</p><button className="guide-example__action" data-example-target>未解説メモのプロンプトをコピー</button><div className="guide-example__memo"><small>保存した疑問</small><p>なぜ月の形は毎日変わって見えるの？</p></div><button className="guide-example__action">AIの回答を取り込む　›</button></>}
