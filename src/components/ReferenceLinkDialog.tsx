...
export function ReferenceLinkDialog({ setIds, questions, onSave, onClose }: { setIds: string[]; questions: Question[]; onSave: (links: ReferenceLink[]) => Promise<void>; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const match = materials.length === 1 ? materials[0] : named.length === 1 ? named[0] : undefined;
    if (match) setSelected(match.id);
  }, [materials, selected, questions]);
  ...
  return <dialog ref={dialog} className="reference-link-dialog" aria-labelledby="reference-link-title" onCancel={event => { event.preventDefault(); if (!busy) onClose(); }}>
    ...
    <div className="reference-link-body">
      <p>ページを自動で読み取りました。資料を確認して登録してください。範囲指定はすべて紐付き、先頭ページへ移動します。</p>
      ...
      {rows.map((row, i) => <label className="reference-link-row" key={row.question.id}><input type="checkbox" disabled={busy || !row.page} checked={checked.has(row.question.id)} onChange={e => setChecked(old => { const next = new Set(old); if (e.target.checked) next.add(row.question.id); else next.delete(row.question.id); return next; })}/><span><strong>{i + 1}. {row.question.question}</strong><small>{row.question.sourcePage || 'Referenceなし'} → {row.page ? `PDF ${row.pages.map(p => p.pdfPage).join('・')}p` : row.reason}</small></span></label>)}
