...
import { changeFolderColor, folderPalette } from '../utils/folderColors';
...
export function LibraryItemActions({ data, kind, id, onSave, onDelete, onAddSet }: Props) {
  ...
  const [mode, setMode] = useState<'rename' | 'move' | 'color' | null>(null);
  ...
  const save = async () => {
    ...
    try {
      ...
      const next = mode === 'color' ? changeFolderColor(data, id, value) : mode === 'move'
  ...
  return <div className="library-actions">
    ...
    {createPortal(<dialog ref={dialogRef} className="library-actions__body library-actions__dialog" aria-label={`${folder?.name ?? set?.title ?? ''}の操作`} onCancel={(event) => { if (busy) event.preventDefault(); }} onClose={() => setOpen(false)}
      ...
      {mode ? <form onSubmit={(event) => { event.preventDefault(); void save(); }}>
        {mode !== 'color' ? <label>{mode === 'rename' ? '名前' : '移動先'}</label> : null}
        {mode === 'color' ? <FolderColorPicker value={folderPalette(value).value} onChange={setValue} disabled={busy} />
        ...
        <button type="submit" disabled={busy || (mode === 'rename' ? !value.trim() : mode === 'color' ? value === (folder?.color ?? 'blue') : value === current)}>{busy ? '保存中…' : mode === 'move' ? 'ここに移動' : '保存'}</button>
        ...
        {folder ? <button type="button" onClick={() => { setValue(folder.color ?? 'blue'); setError(''); setMode('color'); }}>色を変更</button> : null}
