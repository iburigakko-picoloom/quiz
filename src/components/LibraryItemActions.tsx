...
export function LibraryItemActions({ data, kind, id, onSave, onDelete, onAddSet }: Props) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (await onSave(next)) setMode(null);
      else setError('保存できませんでした。もう一度お試しください。');
    } catch (reason) { setError(reason instanceof Error ? reason.message : '保存できませんでした。'); }
    finally { setBusy(false); }
  };
  return <details className="library-actions">
    <summary aria-label={`${folder?.name ?? set?.title ?? ''}の操作`}>…</summary>
    <div className="library-actions__body">
      {mode ? <form onSubmit={(event) => { event.preventDefault(); void save(); }}>
        ...
        {onAddSet ? <button type="button" onClick={onAddSet}>このフォルダに問題セットを追加</button> : null}
        ...
        <button type="button" className="library-danger" onClick={onDelete}>削除</button>
      </>}
    </div>
  </details>;
}
