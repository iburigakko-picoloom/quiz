...
${memoContext ? `
【苦手メモからの問題化】
下の「学習する回答」（取り込み済みの詳細解説）を教材として、その理解を確かめる復習問題を作ってください。「回答の元になった疑問」は学習目的の補足であり、未回答の疑問に答える依頼ではありません。回答に説明された重要事項・判断理由・混同しやすい点を出題し、条件や具体例を変えて理解を確かめてください。各回答を偏りなく扱い、回答の範囲外へ無関係に広げないでください。回答内の誤りや根拠の不明な断定を正解として採用しないでください。
