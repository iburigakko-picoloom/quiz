...
【回答の進め方】
原則1回の回答で完成したJSONファイルを添付してください。作成の予告、方針案、サンプルだけ、作成してよいかの確認は不要です。軽微な未指定事項は資料から判断してください。添付がない・読めない・必須の情報が欠ける場合だけ、必要な確認を1回にまとめてください。判読不能な内容を捏造しないでください。
...
【問題】
一問一主題とし、定義・違い・理由・具体例への適用などを科目に合わせて出題してください。今回の作成条件に従い、数合わせの重複・水増しは避けてください。
誤答は実際に混同しやすい概念や条件に基づき、長さ・文体をそろえてください。無関係な誤答、正解だけ詳しい選択肢、「必ず」を付けただけの誤答は避けてください。
distractorsには明確な追加誤答を6〜12個（最大50個）用意してください。正解・正解の同義語・choicesとの重複は禁止です。確実な候補が足りなければ減らし、誤答候補を捏造しないでください。各問にshuffleChoices:trueを指定してください。Quiz Makeが全正答を保持し、元の誤答と追加候補から必要数を抽選して、正答を含めた並びもランダムにします。複数回答では全正答の根拠が成立し、全誤答候補がその設問では誤りになることを確認してください。
...
【通常解説】
重要語・判断を分ける条件を1問あたり1〜3箇所だけ**太字**で囲んでください（例：**否定語**に注意）。通常解説では赤い太字で表示されます。文全体は強調せず、HTMLや色指定タグは使わないでください。
...
内容のまとまりごとに改行し、根拠・補足・各選択肢の説明を短い段落に分けてください。JSON文字列では改行を\\n、段落間を\\n\\nで表してください。ランダムに表示されない誤答にも依存せず、必要な語句を明記して説明してください。
...
{"setTitle":"問題集名","source":"資料名","questions":[{"id":"q001","category":"主題","question":"問題文","choices":["正解","誤答1","誤答2","誤答3"],"answerIndex":0,"distractors":["追加誤答1","追加誤答2","追加誤答3","追加誤答4","追加誤答5","追加誤答6"],"shuffleChoices":true,"explanation":"**重要な条件**が判断の決め手です。実際の出力では正解の根拠と誤答との差を具体的に説明します。","reference":"資料名 p.3"}]}
大量の問題で1ファイルで扱いきれない場合は、重複表現を削り、必要なら問題数を調整して完結したJSONにしてください。sourceに未収録範囲を短く記載し、全範囲を網羅したふりはしないでください。後半の解説を削って数だけ合わせないでください。
...
【回答の進め方】
原則1回の回答で完成したJSONファイルを添付してください。作成の予告、方針案、サンプルだけ、作成してよいかの確認は不要です。添付がない・読めないなど作業に必須の情報が欠ける場合だけ、必要な確認を1回にまとめてください。
...
【通常解説】
重要語・判断を分ける条件を1問あたり1〜3箇所だけ**太字**で囲んでください（例：**否定語**に注意）。通常解説では赤い太字で表示されます。文全体は強調せず、HTMLや色指定タグは使わないでください。原文の問題文・選択肢には強調を追加しないでください。
...
【出力形式】
ダウンロード可能なUTF-8の「quiz-make.json」を実際に作成・添付してください。ファイル内はJSON本体を1個だけとし、コメント・末尾カンマ・省略記号・コードフェンスは入れません。文字列の改行・引用符はJSONとしてエスケープし、**重要語**を保持してください。見本は実際の問題に置き換えます。チャット本文はファイルへのリンクと短い案内だけにし、JSON全文や作成コードは表示しません。ファイルを作れない環境では、その旨だけを伝え、架空の添付やリンクは作らないでください。
{"setTitle":"過去問集","source":"資料名","questions":[{"id":"q001","category":"主題","question":"原文の問題文","choices":["選択肢1","選択肢2","選択肢3","選択肢4"],"answerIndex":0,"shuffleChoices":false,"explanation":"**重要な条件**が判断の決め手です。実際の出力では正解の根拠と誤答との差を具体的に説明します。","reference":"2025 Q2"}]}
...
大量で1回の1ファイルで扱いきれない場合も、途中で切れたJSONにしないでください。収録済みの問題を完結したJSONにし、sourceに収録範囲と未収録の年度・問題番号を明記してください。「続き」と依頼された場合は未収録分だけを別の完結したJSONファイルで添付してください。全問収録したふりや正答・解説の省略はしないでください。
