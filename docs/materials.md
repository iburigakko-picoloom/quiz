...
- 学習中の資料上部「Referenceから紐付け」から、問題セット内の未登録問題を一括設定できる。単一ページ・範囲・複数の明示的なページ表記を自動解析し、候補を選択済みにする。資料が1つ、または資料名の一致が1つならPDFも候補選択するが、保存は利用者が確認して実行。`p.7-8／1. 解剖・生理`は7・8ページを登録し先頭へ移動（後続の章番号は含めない）。年度や問題番号は推測せず、範囲外を一部だけ登録せず、既存参照は上書きしない。表紙によるページ差を指定可能。元PDFのページ番号から固定ページIDへ変換するため白紙追加や並び替えに影響されない。保存直前に問題のReferenceと既存参照を再確認し、一括永続化成功後だけ画面へ反映。
